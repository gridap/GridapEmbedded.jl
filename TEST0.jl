...
function plot_quantities(A,b,Ω,dΩ;filename="results")
   sol_x = A\b
   xh = FEFunction(X, sol_x)
   uh,ph = xh
   euh = uex-uh
...
push!(wrc[3], cdmix...)
A = assemble_matrix(assem, wrc)
res_stab_dmix = compute_quantities(A,b,dΩ)
...
push!(wrc[3], cdmix...)
A = assemble_matrix(assem, wrc)
res_stab_udivupmixdmix = compute_quantities(A,b,dΩ)
...
push!(wrc[3], cdmix...)
A = assemble_matrix(assem, wrc)
res_stab_ufulldivufullpmixdmix = compute_quantities(A,b,dΩ)
