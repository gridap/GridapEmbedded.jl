...
"""
    returns the L2 projection in the bounding box space of  a function. That is, the L2 projection is defined through

        ∫ (function - Π_{Zbb}(operation(u)) zbb dΩbg_agg_cells = 0   ∀ zbb ∈ Zbb(T), 
...
function setup_L2_proj_in_bb_space(
    ...
    wbb_Ωbg_agg_cells=change_domain_bb_to_agg_cells(wbb,
                                                  ...
                                                  Ωbg_agg_cells,
                                                  agg_cells_to_aggregate)                                               
    # (3) Compute & assemble contributions to LHS of L2 projection
    agg_cells_to_lhs_contribs=get_array(∫(zbb_Ωbg_agg_cells⋅wbb_Ωbg_agg_cells)dΩbg_agg_cells)
    ass_lhs_map=BulkGhostPenaltyAssembleLhsMap(agg_cells_to_lhs_contribs)
    lhs = lazy_map(ass_lhs_map,aggregate_to_local_cells)

    # (4) Compute & assemble contributions to RHS of L2 projection
    agg_cells_rhs_contribs=get_array(∫(zbb_Ωbg_agg_cells⋅func)dΩbg_agg_cells)
    rhs=lazy_map(sum,lazy_map(Broadcasting(Reindex(agg_cells_rhs_contribs)),          aggregate_to_local_cells)) 

    # (5) TO-DO: optimize using our own optimized version Gridap.Fields.Map 
    # of backslash that re-uses storage for lu factors among cells, etc.
    func_proj_Zbb_dofs=lazy_map(\,lhs,rhs)

    # (6) Generate bb-wise array of fields. For each aggregate's bounding box,
    # it provides the l2 projection of func restricted to the cells 
    # included in the bounding box of the aggregate.   
    func_proj_Zbb_array=lazy_map(Gridap.Fields.linear_combination,
                                func_proj_Zbb_dofs,
                                Gridap.CellData.get_data(zbb))

    # (7) Change domain of proj_op_u and proj_func from Ωbb to Ωbg_agg_cells
    func_proj_Zbb_array_agg_cells=lazy_map(Broadcasting(∘),
                                           lazy_map(Reindex(func_proj_Zbb_array),agg_cells_to_aggregate),
                                           ref_agg_cell_to_ref_bb_map)
    func_proj_Zbb_agg_cells = Gridap.CellData.GenericCellField(func_proj_Zbb_array_agg_cells,Ωbg_agg_cells,ReferenceDomain()) 

    func_proj_Zbb_agg_cells
end
...
"""
  ...
  γ ∫( (operation_v(v) - proj_op_v)⊙(func - proj_func) )*dD
...
function bulk_ghost_penalty_stabilization_collect_cell_vector_on_D(dD,
    ...
    # (1) op_v⊙(func-proj_func) term
    op_v_diff_func_contribs=get_array(∫(γ*operation_v(v)⊙(func - proj_func))*dD)
    push!(w, op_v_diff_func_contribs)
    push!(r, dof_ids_v) 

    # (2) proj_op_v⊙(func-proj_func)
    proj_op_v_diff_func_vec_contribs=get_array(∫(γ*(-1.0)*(proj_op_v⊙(func - proj_func)))*dD)
    push!(w, proj_op_v_diff_func_vec_contribs)    
    push!(r, dof_ids_proj_v) 
...
"""
  ...
  γ ∫( (operation_v(v))⊙(func - proj_func) )*dD

  with `v` the test basis functions (which may be components of a MultiField). The operation `operation_v` (e.g. identity or divergence) is applied to v. The L2 projection of `func`, stored as `proj_func`, can be computed via  
  `setup_L2_proj_in_bb_space`. `dD` is the measure of the cut cells or full aggregate, that is `dΩbg_cut_cells` or `dΩbg_agg_cells`. The function `func` is the forcing term appearing on the right hand-side of the PDE. 
...
function bulk_ghost_penalty_stabilization_collect_cell_vector_on_D(dD,
    ...
    # (1) op_v⊙(func-proj_func)
    op_v_diff_func_vec_contribs=get_array(∫(γ*operation_v(v)⊙(func - proj_func))*dD)
    push!(w, op_v_diff_func_vec_contribs)
    push!(r, dof_ids_v) 
