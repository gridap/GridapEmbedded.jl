# TEST1: Darcy test case solved using a zero mean pressure space. In the postprocessing procedure, the solution is corrected for this, so that the mean pressure from the exact solution is matched.
using Gridap
...
## NEWLY ADDED FOR ZM PRESSURE SPACE
# Define root and cut cells.
...
# TO DO: check if this works for order =2!
...
int_nonagg_dof_to_fix = int_nonagg_cells[1] # pick the first interior cell that is not part of an aggregate to be contrained by the zero mean pressure condition.
...
Qzm     = Gridap.FESpaces.ZeroMeanFESpace(spaceWithConstantFixed,Qzm_vol_i,Qzm_vol)
Pzm     = TrialFESpace(Qzm)

# Set up global spaces 
V    = FESpace(Ωact, ReferenceFE(raviart_thomas,Float64,order),conformity=:HDiv)
U    = TrialFESpace(V)
Xzm = MultiFieldFESpace([U, Pzm])
Yzm = MultiFieldFESpace([V, Qzm])
dx_zm    = get_trial_fe_basis(Xzm)
dy_zm    = get_fe_basis(Yzm)
du_zm,dp_zm = dx_zm
dv_zm,dq_zm = dy_zm
...
Ωbg_agg_cell_dof_ids   = get_cell_dof_ids(Xzm,Ωbg_agg_cells)
...
Ωbg_cut_cell_dof_ids   = get_cell_dof_ids(Xzm,Ωbg_cut_cells)
...
function compute_quantities(A,b,dΩ,pex,X)
   cond_A =  cond(Array(A))
   norm_A =  norm(A)
   sol_x = A\b
   ...
   print("zeromean pressure $(sum(∫(ph_zm)dΩ)/area) and corrected mean pressure $(sum(∫(ph)dΩ)/area)")
...
b = assemble_vector(l, Yzm)
#AssertionError: Your are trying to integrate a CellField using a CellQuadrature defined on incompatible triangulations. Verify that either the two objects are defined in the same triangulation or that the triangulaiton of the CellField is the background triangulation of the CellQuadrature.
assem=SparseMatrixAssembler(Xzm,Yzm)

#RHS_DMIX
vec_wr=Gridap.FESpaces.collect_cell_vector(Yzm,l(dy_zm))
push!(vec_wr[1],vecw_dmix...)
push!(vec_wr[2],vecr_dmix...)
vec_b = assemble_vector(assem, vec_wr)

# NO STAB
wrc=Gridap.FESpaces.collect_cell_matrix(Xzm,Yzm,a(dx_zm,dy_zm))
A = assemble_matrix(assem, wrc)
res_nostab = compute_quantities(A,b,dΩ,pex,Xzm)
...
push!(wrc[3], cu...)
A = assemble_matrix(assem, wrc)
res_stab_u = compute_quantities(A,b,dΩ,pex,Xzm)
...
push!(wrc[3], cu_full...)
A = assemble_matrix(assem, wrc)
res_stab_ufull = compute_quantities(A,b,dΩ,pex,Xzm)
...
push!(wrc[3], cp...)
A = assemble_matrix(assem, wrc)
res_stab_p = compute_quantities(A,b,dΩ,pex,Xzm)
...
push!(wrc[3], cp_full...)
A = assemble_matrix(assem, wrc)
res_stab_pfull = compute_quantities(A,b,dΩ,pex,Xzm)
...
push!(wrc[3], cdiv...)
A = assemble_matrix(assem, wrc)
res_stab_div = compute_quantities(A,b,dΩ,pex,Xzm)
...
A = assemble_matrix(assem, wrc)
res_stab_divfull = compute_quantities(A,b,dΩ,pex,Xzm)
...
push!(wrc[3], cpmix...)
A = assemble_matrix(assem, wrc)
# res_stab_pmix = compute_quantities(A,b,dΩ,pex,Xzm)
res_stab_pmix = compute_quantities(A,b,dΩ,pex,Xzm)
...
push!(wrc[3], cdmix...)
A = assemble_matrix(assem, wrc)
res_stab_dmix = compute_quantities(A,vec_b,dΩ,pex,Xzm)
...
push!(wrc[3], cdiv...)
A = assemble_matrix(assem, wrc)
res_stab_updivu = compute_quantities(A,b,dΩ,pex,Xzm)
...
push!(wrc[3], cdiv_full...)
A = assemble_matrix(assem, wrc)
res_stab_ufullpfulldivufull = compute_quantities(A,b,dΩ,pex,Xzm)
...
push!(wrc[3], cdmix...)
A = assemble_matrix(assem, wrc)
res_stab_udivupmixdmix = compute_quantities(A,vec_b,dΩ,pex,Xzm)
...
push!(wrc[3], cdmix...)
A = assemble_matrix(assem, wrc)
res_stab_ufulldivufullpmixdmix = compute_quantities(A,vec_b,dΩ,pex,Xzm)
